...
| `/journal` | 寫學習日記（跑本機 `/journal` skill，一問一答；在日記頻道直接回覆即可） |
| `/ask 問題` | 問 Claude 任何問題，唯讀、可追問 |
| `/reset` | 清掉本頻道的對話記憶 |

> **`/journal` 互動流程**：`/journal` 是多輪問答，靠 `on_message` 聊天頻道機制接住答案 → 頻道名含 `日記`/`journal`（見 `CHAT_NAME_MARKERS`）就能直接打字回覆，不用每則 @。cmd_journal 送出的 prompt 會叮嚀 Claude「一次問一題純文字、問完停下等下一則訊息、不要用 AskUserQuestion」。權限：ALLOWED_TOOLS 已加 `Write/Edit(300_Journal/**)` 讓日記存得進去；Discord 版跳過 git push（存檔就好）。

> `/today`、`/tomorrow` 已於 2026-07-14 移除（改用 `/morning` 早報 + `/todo` 待辦看板）。每日 07:00 自動推播已於 2026-07-14 由「今日行事曆」改為自動跑 `/morning` 早報（行事曆為備援）。

## 直接對話（免打 /ask）

`cogs/claude_bridge.py` 的 `on_message` 監聽器讓你不用每次打 `/ask`：

- **聊天頻道**：頻道名含 `分身` / `bot-room` / `botroom` / `bot_room`（見 `CHAT_NAME_MARKERS`），或 `.env.local` 的 `CHAT_CHANNEL_ID` 指定的頻道 → 直接打字就回。
- **其他頻道**：要 @Tyler_Agent 才回（會自動去掉 @ 標記）。
- 兩者都沿用 `_run_claude`（同一頻道一條 session、唯讀安全模式、`send_long_message` 分段），回話時顯示「輸入中…」。忽略 bot 自己、其他機器人、`!` 前綴、空訊息。
- ⚠️ **需要 Message Content Intent**：這個特權 intent 要先在 Discord 開發者後台（機器人頁 → Privileged Gateway Intents）開啟，程式才用環境變數 `ENABLE_MESSAGE_CONTENT=true` 打開 `intents.message_content`。**順序不能反**：後台沒開就把程式的 intent 打開，bot 會登入失敗被 launchd 一直重啟，所以用 env 當保險。
...
| `ENABLE_MESSAGE_CONTENT` | 設 `true` 才會開 Message Content Intent（需後台先開）| `.env.local` |
| `CHAT_CHANNEL_ID` | （選填）精準指定聊天頻道 ID；不填則靠頻道名關鍵字 | `.env.local` |
| `MORNING_CHANNEL_ID` | 早報 + 待辦提醒的頻道（#📆-早晨推播）；沒設退回 DISCORD_CHANNEL_ID | `.env.local` |
| `NIGHTLY_CHANNEL_ID` | 晚間日誌的頻道（#🌃-晚間回顧）；沒設退回 DISCORD_CHANNEL_ID | `.env.local` |
...
| 台北 | UTC | 內容 | 程式位置 | 送到頻道 |
|------|-----|------|---------|---------|
| 07:00 | 23:00 | `/morning` 早報 | `calendar_local.py` `daily_reminder` | `MORNING_CHANNEL_ID`（#📆-早晨推播）|
| 20:00 | 12:00 | 待辦到期分級提醒 | `todo_reminder.py` `due_check` | `MORNING_CHANNEL_ID`（#📆-早晨推播）|
| 22:00 | 14:00 | `/nightly` 晚間日誌 | `claude_bridge.py` `nightly_journal` | `NIGHTLY_CHANNEL_ID`（#🌃-晚間回顧）|
| 每 30 分 | — | 班表變動通知 + 起飛前提醒排程 | `calendar_local.py` `schedule_sync` | `DISCORD_CHANNEL_ID`（#✈️-班表推播）|

> 各推播分流到不同頻道：每日生活（早報/待辦）→ 早晨推播；晚間日誌 → 晚間回顧；飛行相關 → 班表推播。`MORNING_CHANNEL_ID` / `NIGHTLY_CHANNEL_ID` 沒設時都退回 `DISCORD_CHANNEL_ID`（2026-07-16 分流）。

> **每日推播：補送 + 位置感知（2026-07-18 改寫）**：三個每日推播都改成「每隔幾分鐘檢查、到當地時段且今天沒送過就送」，取代原本固定 UTC 時間的一發式 loop。這樣 (a) 排程當下 bot 離線（Mac 睡醒/斷網）也會在連回來後補送；(b) 早報會跟著「你目前所在地」用當地時區送。
>
> - **去重狀態**：`state/daily_pushes.json` = `{push_key: 最後成功送出的當地日期}`。共用 `push_due(key, start_hour, until_hour, tz=None)` 判斷該不該送；`_mark_push_sent(key, date)` 記錄。
> - **早報**（`calendar_local.morning_check`，每 10 分）：`tz=None` → 用 `current_location()` 推斷的所在地時區，窗 07:00–11:00 當地。
> - **待辦**（`todo_reminder.due_check`，每 15 分）：固定台北，窗 20:00–23:00。**晚間日誌**（`claude_bridge.nightly_journal`，每 15 分）：固定台北，窗 22:00–24:00。
> - **位置推斷 `current_location()`**：① 先看 `state/location_override.json`（手動指定，如 `{"tz":"Asia/Bangkok","airport":"BKK","expires":"2026-07-19"}`，過期自動失效）→ ② 否則從 EVA 工作班表推斷（最後一班已起飛航班的目的地，`_detect_location_airport()`，快取 30 分）→ ③ 都沒有就母基地 TPE。⚠️ 個人旅遊（員工票、不在工作班表）推斷不到，要用 override 手動指定。
